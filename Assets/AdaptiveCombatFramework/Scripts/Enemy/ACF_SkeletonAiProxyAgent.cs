...
namespace AdaptiveCombatFramework {
    public class EnemyAIProxyAgent : Agent
    {
        [Header("System References")]
        [Tooltip("The physical body script this agent is controlling.")]
        public SkeletonAI skeletonBody;
        [Tooltip("Stats of the controlled agent.")]
        public CharacterStats myStats;
        [Tooltip("Stats of the opponent.")]
        ...
        [Header("Reward Scales (AI Motivation)")]
        [Tooltip("Applied every fixed step. Small negative value to encourage the AI to finish the fight efficiently rather than idling.")]
        public float stepPenalty = -0.0005f;

        [Tooltip("Reward granted when the AI stays within its preferred combat range and faces the player.")]
        public float engagementReward = 0.001f;

        [Tooltip("Penalty applied if the AI wanders too far outside its sensor radius or preferred range.")]
        public float disengagePenalty = -0.001f;

        [Tooltip("Bonus reward for successfully triggering an attack while the player is actually within that attack's optimal range.")]
        public float inRangeAttackReward = 0.003f;

        [Tooltip("Penalty applied if the AI attempts an attack while being far out of range or facing the wrong way.")]
        public float badAttackChoicePenalty = -0.003f;

        [Tooltip("Multiplier for damage dealt to the player. Damage is normalized (0 to 1) based on player max health.")]
        public float damageDealtRewardScale = 0.75f;

        [Tooltip("Multiplier for damage taken from the player. Higher values make the AI more 'careful' about getting hit.")]
        public float damageTakenPenaltyScale = 1.0f;

        [Tooltip("Large one-time reward granted when the player's health reaches zero.")]
        public float winReward = 1.5f;

        [Tooltip("Large one-time penalty applied when the AI's own health reaches zero.")]
        public float lossPenalty = -1.5f;

        [Tooltip("Multiplier for the style-match reward generated by the MultiGAIL critics. Controls how much the AI cares about 'looking' like the target persona.")]
